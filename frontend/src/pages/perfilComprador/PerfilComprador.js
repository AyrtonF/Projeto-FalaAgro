...
const PerfilComprador = () => {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                const response = await axios.get('https://backend-final-ytc2.onrender.com/user/', {
                    ...
                    <Col xs={12} md={6} lg={3} className="mb-4">
                        <Card className="h-100">
                            <Card.Body>
                                ...
                                <Link to="/EditarUser" className="btn btn-danger w-100">Redefinir/Excluir</Link>
                            </Card.Body>
                        </Card>
                    </Col>
                    {user && user.store[0] && (
                        <Col xs={12} md={6} lg={3} className="mb-4">
                            <Card className="h-100">
                                <Card.Body>
                                    ...
                                    <Link to="/perfilLoja" className="btn btn-warning w-100">Ver Perfil da Loja</Link>
                                </Card.Body>
                            </Card>
                        </Col>
                    )}
