...
const PaginaProduto = () => {
  ...
  const { adicionarProduto, limparCarrinho, sucesso, error } =
    useContext(CarrinhoContext); // Usar o contexto do carrinho
  ...
  useEffect(() => {
    const fetchProduto = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3333/product/${productId}`
        );
        // Verifica se o produto tem tags, caso contrário, adiciona uma tag padrão
        if (response.data.tags && response.data.tags.length === 0) {
          response.data.tags.push("Nenhuma categoria");
    ...
    const fetchLoja = async (storeId) => {
      try {
        const response = await axios.get(
          `http://localhost:3333/store/${storeId}`
        );
        setLoja(response.data);
      } catch (error) {
        console.error("Erro ao buscar a loja:", error);
  ...
  const handleAdicionarAoCarrinho = () => {
    ...
    if (!token) {
      ...
      if (!sucesso) {
        if (
          window.confirm(
            "O carrinho contém produtos de outra loja. Deseja substituir o carrinho com o novo produto?"
          )
        ) {
          limparCarrinho();
      ...
      <div className="row">
        ...
        <div className="col-lg-6 col-md-12">
          <div className="produto-info">
            ...
            {produto.dimensoes && (
              <p className="produto-dimensoes">
                <strong>Dimensões:</strong> {produto.dimensoes}
              </p>
            )}
            <p className={`produto-status ${getStatusClass(produto.status)}`}>
              <strong>Status:</strong> {produto.status || "Ativo"}
            </p>
            <div className="produto-tags">
              <strong>Tags:</strong>{" "}
              {produto.tags.map((tag) => (
                <a key={tag} href="#" className="produto-tag">
                  {tag}
                </a>
              ))}
            </div>
            <button
              className="btn btn-primary btn-adicionar-carrinho"
      ...
      <div className="row loja-info">
        <div className="col-md-12">
          <div className="loja-perfil">
            <img
              src={loja && loja.imagem ? loja.imagem : lojaImg}
              alt="Imagem da Loja"
              className="loja-imagem"
            />
            <div className="loja-detalhes">
              <h3 className="loja-nome">
                {loja ? loja.name : "Não Carreguei"}
              </h3>
